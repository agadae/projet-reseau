...
############################ Graph Data ########################################
    ...
    def interpret(self, packet):
        from Tiles.Bob.bob import Bob
        print("depuis l'interpret")
        print(packet.ID)
		# Interpret the packet and return a list of instructions
        match packet.ID:
            case "SPAWN_B":
                print("[<--] Spawned Unit : " + packet.data)
                datatab = packet.data.split(";")
                tile = self.getMap()[int(datatab[1])][int(datatab[2])]
                Bob = Bob()
                Bob.id=datatab[0]
                Bob.owner=packet.PNAME
                Bob.spawn_online(tile)
                print("I received the spawning packet")

            case "DIE_B":
                print("[<--] Killed Unit : " + packet.data)
                datatab = packet.data.split(";")
                RtdBob = Bob()
                for bob in self.listBobs:
                    if bob.id == datatab[0]:
