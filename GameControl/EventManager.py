...
from network.pytoc import Packet
from network.pytoc import *

fichier_communication = "ip.txt"


# Ouvrir le descripteur de fichier en écriture
writeDesc = os.open(fichier_communication, os.O_WRONLY)

# Vérifier si l'ouverture a réussi
if writeDesc == -1:
    print("Erreur : impossible d'ouvrir le descripteur de fichier.")
else:
    print("validé !!! ")
...
pg.mixer.music.set_volume(0)  # Le volume de 0.0 à 1.0
...
create_online_button_rect = pg.Rect((screen.get_width() - button_width) // 2 - 150, 300, button_width, button_height)
join_online_button_rect = pg.Rect((screen.get_width() - button_width) // 2 - 150, 400, button_width, button_height)
...
def open_create_online_game(screen, clock):
    global return_to_menu, create_online_game_open
    from GameControl.game import Game
    game = Game.getInstance(screen,clock)
    game.activate_multiplayer_host()

    # Déclaration des rectangles des boutons de chargement
    button_width, button_height = 300, 50
    start_game_button = pg.Rect((screen.get_width() - button_width) // 2, 200, button_width, button_height)
    ...
    while True:
        for event in pg.event.get():
            if event.type == pg.QUIT:
                ...
                if start_game_button.collidepoint(event.pos):
                    ...
                    game.deactive_multiplayer_host()
                    create_online_game_open = False
                    return_to_menu = True
                    return
        ...
        # Center the load buttons horizontally and vertically
        draw_transparent_button("START GAME", start_game_button, 128)
        draw_transparent_button("BACK", back_button_rect, 128)
        # Ajoutez d'autres boutons de chargement ici
    
        game.count += 1
        if (game.count == 30) : # changer si trop rapide ou trop long
            '''p = receive_string(setting.lecture_fd, False)
            if p :
                print(p.stringify())'''
            game.count = 0    
            pg.display.flip()
...
# Déclaration de la variable globale pour stocker l'adresse IP et le port
...
def extract_ip(packet):
    # Extract IP from Packet object
    if isinstance(packet, Packet):
        return packet.data.split(';')[0]  # Assuming IP is the first part before the ';'
    else:
        return None  # Handle error or return None if packet format is incorrect


def open_join_online_game(screen, clock):
    from GameControl.game import Game
    global return_to_menu, create_online_game_open, ip_address_input
    game = Game.getInstance(screen,clock)
    ...
    ip_surface = pg.Surface((300, 50))
    ip_surface.fill(WHITE)
    ip_rect = ip_surface.get_rect(topleft=(100, 200))
    ...
    while True:
        for event in pg.event.get():
            if event.type == pg.QUIT:
                ...
                # Bouton pour soumettre l'IP et le port
                if submit_button_rect.collidepoint(event.pos):
                    # Traitez l'IP et le port ici, par exemple, avec une fonction de connexion réseau
                    txt = "JOIN " + setting.pseudo + " " + ip_address_input
                    print(txt)
                    send(txt, setting.ecriture_fd)
                    txt=""
                ...
                # Vérifier si la souris est enfoncée dans les rectangles de saisie
                elif ip_rect.collidepoint(event.pos):
                    input_active = "ip"


            elif event.type == pg.KEYDOWN:
                # Vérifier quel champ de saisie est actif
                if input_active == "ip":
                    if event.key == pg.K_BACKSPACE:
                        ip_address_input = ip_address_input[:-1]
                    else:
                        ip_address_input += event.unicode

        count += 1
        if (count == 5) : # changer si trop rapide ou trop long
            p = receive_string(setting.lecture_fd, False)
            print(p)
            if p :
                match p.ID:
        ...
        # Dessiner les champs de saisie pour l'adresse IP
        screen.blit(ip_surface, ip_rect)
        ip_text = font.render("Adresse IP: " + ip_address_input, True, BLACK)
        screen.blit(ip_text, (ip_rect.x + 10, ip_rect.y + 10))

        # Dessiner le bouton de soumission
        submit_button_rect = pg.Rect(200, 400, 200, 50)
        pg.draw.rect(screen, GREEN, submit_button_rect)
        submit_text = font.render("Submit", True, BLACK)
        screen.blit(submit_text, (250, 410))
...
def show_menu(screen, clock):
    global selected_value_index, grid_value_rects, grid_dict, settings_open, return_to_menu, load_open, create_online_game_open
    ...
    create_online_button_rect = pg.Rect((screen.get_width() - button_width) // 2, 400, button_width, button_height)
    join_online_button_rect = pg.Rect((screen.get_width() - button_width) // 2, 500, button_width, button_height)
    settings_button_rect = pg.Rect((screen.get_width() - button_width) // 2, 600, button_width, button_height)
    quit_button_rect = pg.Rect((screen.get_width() - button_width) // 2, 700, button_width, button_height)
    ...
    while True:
            ...
            if event.type == pg.QUIT:
                ...
                if load_open:
                    if back_button_rect.collidepoint(event.pos):
                        load_open = False
                        return_to_menu = True
                if create_online_game_open:
                    if create_online_button_rect.collidepoint(event.pos):
                        return_to_menu = False
                        create_online_game_open = True
                else:
                    if play_button_rect.collidepoint(event.pos):
                        ...
                        settings_open = True
                        open_settings()
                    elif create_online_button_rect.collidepoint(event.pos):
                        return_to_menu = False
                        create_online_game_open = True
                        if open_create_online_game(screen,clock) == 0:
                            return 0
                    elif quit_button_rect.collidepoint(event.pos):
                        pg.quit()
                        sys.exit()
                    elif join_online_button_rect.collidepoint(event.pos):
                        return_to_menu = False
                        create_online_game_open = True
                        if open_join_online_game(screen, clock)==0:
                            return 0
        ...
        elif create_online_game_open:
            open_create_online_game(screen,clock)

        else:
            draw_transparent_button("NEW GAME", play_button_rect, 128)
            draw_transparent_button("CREATE ONLINE GAME", create_online_button_rect, 128)
            draw_transparent_button("JOIN ONLINE GAME", join_online_button_rect, 128)
                ...
                # for bob in coord[0].getBobs():
                    if ( mouse_y - 150 >= 0 ):
                        if ( mouse_x - 50*nb < 0 ):
                            ...
                            i = 0
                            for bob in coord[0].getBobs():
                                draw_text(pauseSurface, f"Owner: {bob.owner}",15,(0,0,0) , ((mouse_x - camera.scroll.x +50+ 100*i + 5 , mouse_y - camera.scroll.y -50 + 65)))
                            ...
                            i = 0
                            for bob in coord[0].getBobs():
                                draw_text(pauseSurface, f"Owner: {bob.owner}",15,(0,0,0) , (mouse_x - camera.scroll.x -50 * nb + 100*i + 5 , mouse_y - camera.scroll.y -150 + 65))
                        ...
                        i = 0
                        for bob in coord[0].getBobs():
                            draw_text(pauseSurface, f"Owner: {bob.owner}",15,(0,0,0) , ((mouse_x - camera.scroll.x -50* nb + 100*i + 5 , mouse_y - camera.scroll.y -50 + 65)))
