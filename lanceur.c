...
#define PORT 1234
#define MAX_CLI 2 // 3 autres joueurs en plus de nous
...
void join_game(char ip[IP_LEN + 1]) {
	...
	int val = 1;
	retour = setsockopt(players[0].sock, SOL_SOCKET, SO_REUSEADDR, &val, sizeof(val));
	if (retour == ERROR) {
		close(fd_py_to_c[TUBE_LECT]);
		close(fd_c_to_py[TUBE_ECRI]);
	...
	val = TRUE;
	retour = setsockopt(players[0].sock, SOL_SOCKET, SO_REUSEPORT, &val, sizeof(val));
	if (retour == ERROR) {
		close(fd_py_to_c[TUBE_LECT]);
		close(fd_c_to_py[TUBE_ECRI]);
		close(players[0].sock);
	...
	if (strcmp(buff, "FIRST") == 0) {
		...
		int val = 1;
		retour = setsockopt(players[1].sock, SOL_SOCKET, SO_REUSEADDR, &val, sizeof(val));
		if (retour == ERROR) {
			close(fd_py_to_c[TUBE_LECT]);
			close(fd_c_to_py[TUBE_ECRI]);
			close(players[1].sock);
			error("Erreur de setsockopt!");
		}
		
		val = TRUE;
		retour = setsockopt(players[1].sock, SOL_SOCKET, SO_REUSEPORT, &val, sizeof(val));
		if (retour == ERROR) {
			close(fd_py_to_c[TUBE_LECT]);
			close(fd_c_to_py[TUBE_ECRI]);
			close(players[1].sock);
...
void create_serv() {
	...
	if (retour == ERROR) {
		...
		close(fd_c_to_py[TUBE_ECRI]);
		if (is_serv_launched) close_serv();
		error("Erreur de bind de la socket!1");
	}
	...
	if (retour == ERROR) {
		...
		close(fd_c_to_py[TUBE_ECRI]);
		if (is_serv_launched) close_serv();
		error("Erreur de listen de la socket!2");
	}
