        ...
        for event in pg.event.get():
            if event.type == pg.QUIT:
                ...
                if submit_button_rect.collidepoint(event.pos):
                    ...
                    print("IP Address:", ip_address_input)

                    packet = Packet("CONN", "OUT", "player1", ip_address_input)


                    ip = extract_ip(packet)

                    if ip:
                        print("On appelle le send de EventManager")
                        send(ip, writeDesc)
                    else:
                        print("[!] Error: Failed to extract IP from packet.")
            ...
            if event.type == pg.QUIT:
                ...
                if create_online_game_open:
                    ...
                    if play_button_rect.collidepoint(event.pos):
                        ...
                        return_to_menu = False
                        create_online_game_open = True
                        open_join_online_game(screen, clock)
