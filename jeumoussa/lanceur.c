...
#define TUBE_LECT 0
#define TUBE_ECRI 1
#define PACKET_SIZE 1024 // a determiner
#define PORT 1234
#define MAX_CLI 10 // 3 autres joueurs en plus de nous
...
void launch_communication() {
    // On ferme les parties des tubes que l'on n'utilisera pas
    ...
    bzero(buff, PACKET_SIZE + 1); // Initialiser le buffer avec des zéros

    // Lire l'adresse IP depuis le fichier et la stocker dans le buffer
    char *ip = read_ip_from_file();
    if (ip != NULL) {
        strncpy(buff, ip, sizeof(buff) - 1);
        buff[sizeof(buff) - 1] = '\0';
        printf("Adresse IP lue dans launch_communication : %s\n", ip);
        printf("Contenu du buffer initial : %s\n", buff);
    } else {
        fprintf(stderr, "Erreur : Impossible de lire l'adresse IP depuis le fichier.\n");
    }

    while (TRUE) {
        // Créer l'ensemble de file descriptor sur lesquels on attend des messages
        int max_fd;
        fd_set set = create_set(&max_fd);

        // Affichage du contenu du buffer avant l'appel à select
        printf("Contenu du buffer avant select : %s\n", buff);
        ...
        // Regarder qui veut nous envoyer un message (tube ou socket)
        if (FD_ISSET(fd_py_to_c[TUBE_LECT], &set)) { // Le programme Python veut envoyer un message
            bzero(buff, PACKET_SIZE + 1); // Réinitialiser le buffer avant de lire le message
            recuperer_packet(buff, fd_py_to_c[TUBE_LECT]);
            // Afficher le contenu du buffer après réception d'un message
            printf("Contenu du buffer après réception d'un message Python : %s\n", buff);
            gerer_py_mess(buff);
        }
        ...
        if (is_serv_launched) {
            ...
            for (int i = 0; i < MAX_CLI; i++) {
                if (players[i].sock != ERROR && FD_ISSET(players[i].sock, &set)) {
                    bzero(buff, PACKET_SIZE + 1); // Réinitialiser le buffer avant de lire le message
                    recuperer_packet(buff, players[i].sock);
                    printf("Message reçu du joueur %s : %s\n", players[i].pseudo, buff);
                    gerer_c_mess(buff, i);
                }
            }
        }
...
char *read_ip_from_file() {
    static char ip_buffer[IP_LEN + 1]; // Déclarer une variable statique pour éviter la déréférence
    FILE *fp = fopen("ip.txt", "r");
    if (fp == NULL) {
        perror("Erreur lors de l'ouverture du fichier ip.txt");
        return NULL; // Retourner NULL en cas d'erreur
    }
    
    if (fgets(ip_buffer, IP_LEN + 1, fp) == NULL) {
        fclose(fp);
        perror("Erreur lors de la lecture de l'adresse IP");
        return NULL; // Retourner NULL en cas d'erreur
    }
    
    fclose(fp);
    // Supprimer le saut de ligne à la fin s'il y en a un
    char *newline = strchr(ip_buffer, '\n');
    if (newline) *newline = '\0';

    printf("L'adresse IP lue : %s\n", ip_buffer); // Ajout du print pour afficher l'adresse lue
    
    return ip_buffer; // Retourner l'adresse IP lue
}
...
void join_game(char ip[IP_LEN + 1]) {
	...
	printf("ip lue dans join_game: %s ",ip);
	players[0].port = PORT;
